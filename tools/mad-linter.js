 ...
 *  inventó. El chequeo [H] detecta eso: mismo ID con título distinto acá y allá.
 *
...
function similitudTitulos(a, b) {
  ...
  return comunes / (setA.size + setB.size - comunes);
}
...
function checkTitleConsistency(files) {
  ...
  for (const [p, text] of files) {
    if (CONFIG.TITLE_CHECK_SKIP.test(base(p))) continue;
    const lines = text.split(/\r?\n/);
...
function lint(paths) {
  ...
  return {
    ...
    catalog, anexoEvents, eventsNotInCatalog, similarGroups, titleDivergences,
  };
}
  ...
  if (r.titleDivergences.length) {
    for (const d of r.titleDivergences) {
      ...
      for (const v of d.variantes) {
        const tc = v.titulo.length > 70 ? v.titulo.slice(0, 70) + '...' : v.titulo;
        console.log('        - "' + tc + '"  [' + v.archivos.join(', ') + ']');
      }
    }
